...
def main():
    ...
    print("\nMost-flexible INTERNAL WT regions (termini 29-50, 290-293 excluded):")
    internal = (resid > 50) & (resid < 290)      # drop flailing chain ends
    idx_internal = np.where(internal)[0]
    order = idx_internal[np.argsort(wt_m[idx_internal])[::-1][:10]]
    for i in order:
        reg = next((n for n,(a,b) in IS_REGIONS.items() if a <= resid[i] <= b), "-")
        print(f"  res {resid[i]} ({reg}): WT RMSF {wt_m[i]:.2f} A")
    print("\n-> these are the real unfolding-prone sites. Is the disulfide (79,153) among them?")
